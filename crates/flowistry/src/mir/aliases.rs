...
impl Aliases<'tcx> {
  ...
  fn compute_loans_loc(
    ...
    // For all e = &'a p in body: contains('a, p).
    {
      ...
      for (region, loan, li) in body_with_facts.input_facts.loan_issued_at.clone() {
        debug!(
          "loan {:?} at region: {:?} issued at location: {:?}",
          loan,
          region,
          body_with_facts.location_table.to_location(li)
        );
      ...
      for (loan, li) in body_with_facts.input_facts.loan_killed_at.clone() {
        debug!(
          "loan {:?} killed at location: {:?}",
          loan,
          body_with_facts.location_table.to_location(li)
        );
      ...
      for (li, loan) in body_with_facts.input_facts.loan_invalidated_at.clone() {
        debug!(
          "loan {:?} invalidated at location: {:?}:",
          loan,
          body_with_facts.location_table.to_location(li)
        );
